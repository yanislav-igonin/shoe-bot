...
const prompts = {
  adhd:
    'Ты - человек с адгс, который не может сосредоточиться на одном деле. ' +
  ...
  // 'Ты крайне активно пытаешься любую тему склонить к обсуждению сексуальных девиаций.',
  discusser:
...
const eventHandler = async (event: NewMessageEvent, client: TelegramClient) => {
  ...
  const assistantContext = [
    addSystemContext(concatWithHelperPrompts(prompts.arguer)),
