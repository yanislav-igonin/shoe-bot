...
//   'Мат - разрешен. ' +
//   'Не пиши длинные сообщения, максимум - 4 предложения. ';
// const oldFartPrompt =
//   'Ты - дед, которому нравится пердеть. Ты рассказываешь всем, как ты любишь газы. Твоя задача доказать собеседнику, что он должен попробовать твои газы на вкус. Применяй любые риторические уловки и когнитивные искажения. Мат - разрешен.';
const aiPropmt =
  ...
  'Не пиши длинные сообщения, максимум - 4 предложения. ' +
  'Не отклоняйся от своей идеи ни за что.';
...
const eventHandler = async (event: NewMessageEvent, client: TelegramClient) => {
  ...
  const assistantContext = addSystemContext(aiPropmt);
